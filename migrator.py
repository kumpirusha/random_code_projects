...
def migrate_columns(parquet_fn_path: str, src: S3Uri, migration_config: List[ColumnMigration]):
    ...
    for cm in migration_config:
        ...
        if original_column_schema == cm.to_type:
            ...
            pq_convert = pq_dataset.column(cm.column).cast(cm.to_type)
            ...
            logger.info(f'Column {cm.column} in {src} matches from_type {cm.from_type}, successfully migrated  '
                        f'to {cm.to_type}')
...
def migrate_parquet_multi(s3: s3fs.S3FileSystem, src: S3Uri, dst: S3Uri,
                          migration_configs: List[ColumnMigration]):
    t = time.time()
    success = True
    ...
    # Migrate columns
    try:
        migrated_pqt = migrate_columns(migrated_pqt, src, migration_configs)
    ...
    if migrated_pqt is not None:
        ...
        pq.write_table(table=migrated_pqt, where=outfn)
        # Upload back to S3
...
def get_etl_part(table_name: str, u: S3Uri):
    """
    >>> get_etl_part('corporate_monitoring',
    ... S3Uri('s3://de-etl-data/data/etl/corporate_monitoring/20201001/'
    ... '2021-08-11 22:37:31.615724/year=2020/month=10/date=2020-10-01/'))
    'corporate_monitoring/20201001/2021-08-11 22:37:31.615724/year=2020/month=10/date=2020-10-01/'
    """
    tokns = str(u).split('/')
...
def get_partition_uris_s3_path(s3_loc: S3Uri, dest_base: S3Uri, backup_base: S3Uri):
    ...
    parquet_map = [('/'.join(pqfn.key.split('/')[:-1]) + '/',
                    S3Uri('s3://' + buckt.name + '/' + '/'.join(pqfn.key.split('/')[:-1]) + '/')) for pqfn in objects]
    return [PartitionMigrationUris(p[1], dest_base.append(p[0]), backup_base.append(p[0])) for p in
            parquet_map]


def para_migrate(s3, migration_uris, cols, hot_run, workers, backwards) \
        -> Tuple[bool, List[Tuple[str, str]], List[Tuple[str, str]]]:
    lock, to_copy, encountered_errors = Lock(), set(), set()

    def run(puris):
        def f():
            migration_success, src_dst_pairs = migrate_partition(s3, puris, cols, hot_run)
            with lock:
    ...
    tasks = [run(puri) for puri in sorted(list(migration_uris), key=lambda x: x.partition_location, reverse=True)]
    if backwards:
        tasks = tasks[::-1]
...
def glue_based_migration(dest_base: S3Uri, backup_base: S3Uri, column_migs: List[ColumnMigration],
    ...
    t = time.time()
    logger.info(f'Migrating parquets in {db}.{table_name} Glue table (DB loc {db_loc}), columns {column_migs}, '
    ...
    if not success:
        ...
        logger.info(f'Migration of {db}.{table_name} for {column_migs} done  in {int(time.time() - t)} seconds')
...
def process_migration_uris(migration_uris: List[PartitionMigrationUris],
    ...
    s3 = s3fs.S3FileSystem(
        session=Session(region_name=REGION, profile_name='elevated') if is_sensitive else Session(region_name=REGION),
        config_kwargs={'max_pool_connections': workers * 4})
    para_run_success, to_copy_pairs, errors_pairs \
        = para_migrate(s3, migration_uris, column_migs, hot_run, workers, backwards)
    do_copy = True
    if not para_run_success:
        do_copy = False
        logger.error(f'Encountered errors in parallel run of {migration_uris} using {column_migs}, will not copy over')
    elif errors_pairs:
        do_copy = False
...
def migrate_partition(s3, puris: PartitionMigrationUris, cols: List[ColumnMigration], hot_run: bool = False) \
    ...
    for pqt in s3.ls(str(puris.partition_location)):
        ...
        migration_performed, success = migrate_parquet_multi(s3, src, dst, cols)
